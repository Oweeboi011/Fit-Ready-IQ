...
4. **npm audit** — check for high/critical vulnerabilities in src/frontend/
5. **Type errors** — note if `src/frontend/tsconfig.tsbuildinfo` is stale (check mtime vs source files)
6. **Secrets check** — verify `.env.local` exists in src/frontend/ and `.env` is not accidentally populated
